...
test_auto_switch() {
	...
	if [ "$status" == 0 ]; then
		...
		echolog "Automatic switching detection: ${id}【$(config_n_get $now_node type): [$(config_n_get $now_node remarks)]] Abnormal, switch to the next backup node for detection!"
		local new_node
